...
author: "Chris Zuidema, Elena Austin for ENVH 556"
...
filenames <- list.files(data_dir, recursive = T) %>% str_subset("car1")
...
In order to capture the change in ultrafine particles, two ultrafine particle counters (PTrak) instruments were used. One instrument was operated under standard conditions and reported the total particle number concentration of particles (#/cc) for all particles with diameters between 20 nm - 1 um. The second Ptrak operated using a screened inlet. The mesh size used and the number of layers of mesh were selected to intercept the smallest particles as they entered the system and changed the lower particle size to 36 nm. The difference between the unscreened and screened Ptrak instruments provided an estimate the particle concentration of 20-36 nm particles. This was verified using a particle Scanning Mobility Particle Sizer Spectrometer.
...
Next, we'll use the P-Trak data to explore the idea of pollutant ratios by plotting the proportion of smallest particles (20-36 nm to the total cocentration of UFP particles).
...
<!-- I usually identify peak events through pca identification of sources. I'm not sure we can do it here with so few elements. The other option might be to look at the rate of change is CO2 to identify peaks, but again not necessarily a good fit for this lab -->
...
ts_data <- ts_data %>% 
  mutate(co2_lag = lead(co2_adj), 
         ...
         co2_diff_sq = replace(co2_diff_sq, co2_diff_sq> 
                                  quantile(co2_diff_sq, 0.95,  na.rm=T), 
                                quantile(co2_diff_sq, 0.95,  na.rm=T)),
...
ggplot(ts_data, aes(datetime, co2_adj, color = (roll_co2_diff)))+ 
